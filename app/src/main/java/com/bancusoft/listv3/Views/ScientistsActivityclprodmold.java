...
import com.bancusoft.listv3.Helpers.MyAdapterclprodmold;
...
import com.bancusoft.listv3.Retrofit.ResponseModelCl_prodmold;
...
public class ScientistsActivityclprodmold extends AppCompatActivity
        implements SearchView.OnQueryTextListener,MenuItem.OnActionExpandListener{

    //We define our instance fields
    private RecyclerView rv;
    private MyAdapterclprodmold mAdapter;
    private LinearLayoutManager layoutManager;
    public ArrayList<Cl_prodmold> allPagesScientists = new ArrayList<>();
    private List<Cl_prodmold> currentPageScientists;
    ...
    private void initializeViews() {
        mProgressBar = findViewById(R.id.mProgressBarLoad_cl_prodmold);
        mProgressBar.setIndeterminate(true);
        Utils.showProgressBar(mProgressBar);
        rv = findViewById(R.id.mRecyclerView_cl_prodmold);
    }
    ...
    private void setupRecyclerView() {
        ...
        mAdapter = new MyAdapterclprodmold(this, allPagesScientists);
    ...
    private void retrieveAndFillRecyclerView(final String action, String queryString,
        ...
        Call<ResponseModelCl_prodmold> retrievedData;

        if (action.equalsIgnoreCase("GET_PAGINATEDCLPROD")) {
            retrievedData = api.search_cl_prodmold("GET_PAGINATEDCLPROD", queryString, start, "100");
            Utils.showProgressBar(mProgressBar);
        } else if (action.equalsIgnoreCase("GET_PAGINATED_SEARCHCLPROD")) {
            Utils.showProgressBar(mProgressBar);
            retrievedData = api.search_cl_prodmold("GET_PAGINATED_SEARCHCLPROD", queryString, start, "100");
        } else {
            Utils.showProgressBar(mProgressBar);
            retrievedData = api.retrieveclprodmold();
        }
        ...
        retrievedData.enqueue(new Callback<>() {
            ...
            @Override
            public void onFailure(@NonNull Call<ResponseModelCl_prodmold> call, @NonNull Throwable t) {
                Utils.hideProgressBar(mProgressBar);
                Log.d("RETROFIT", "ERROR: " + t.getMessage());
                Utils.showInfoDialog(ScientistsActivityclprodmold.this, "ERROR", t.getMessage());
            }
        });
    }
    /**
        ...
        rv.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(@NonNull RecyclerView rv, int dx, int dy) {
                ...
                if (isScrolling && (currentScientists + scrolledOutScientists ==
                    ...
                    if (dy > 0) {
                        // Scrolling up
                        retrieveAndFillRecyclerView("GET_PAGINATEDCLPROD",
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id==R.id.action_new_cl_prodmold){
        ...
        if (id==R.id.action_new_en_cl_prodmold){
        ...
        if (id==R.id.action_new_ru_cl_prodmold){
    ...
    public boolean onQueryTextChange(String query) {
        retrieveAndFillRecyclerView("GET_PAGINATED_SEARCHCLPROD", query, "0");
        return false;
    }
...
//-----------------------------
        ...
        setupRecyclerView();
        retrieveAndFillRecyclerView("GET_PAGINATEDCLPROD", "", "0");
    }
}
